...
void pre(){
	...
	for(int i=1;i<=n;i++){
		...
		int len=s[i].length();
		for(int j=1;j<=len;j++){
			Hash[i][j]=(Hash[i][j-1]*base+(s[i][j-1]-96));
		}
	}
}

ull gethash(int i,int a,int b){
	return (Hash[i][b]-Hash[i][a-1]*power[b-a+1]);
}
...
int f(int i,int cnt){
	...
	for(int k=1;k<=n;k++){
		...
		ull txt=gethash(source,i+1,i+len);
		ull pat=gethash(k,1,len);
